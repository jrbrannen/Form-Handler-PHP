...
//Model-Controller Area.  The PHP processing code goes in this area.
    ...
    if(isset($_POST['submit'])){
        ...
        // If honeypot is not empty reloads form page
        $message = $_POST['message'];
        if (!empty($message)){          // could be made to be empty instead of !empty
            header("Refresh:0");
        }else{      // If honeypot is not empty these variable will be set in order they are listed on the form
            $fName = $_POST['first_name'];
            $lName = $_POST['last_name'];
            $subType = $_POST['sub_type'];
            $offers = $_POST['spec_offers'];
                if(isset($offers)){     // Checks to see if the offers checkbox is checked or not checked
                    $offers = "Yes";
                }else{
                    $offers =  "No";
                }
...
<!DOCTYPE html>
    ...
    <body>
        ...
        <section>
            <?php
                // PHP response message if form was submitted (view)
                if(isset($_POST['submit'])){
                ...
                // Displays form if form has not been submitted (view)
            ?>
            ...
            <form id="newsletter-form" name="newsletter_form" method="post" action="form-handler-homework.php">
                ...
                <!-- this is honeypot only commenting on it for instructor reference -->
                <p class="formField">
        ...
        <footer>
            ...
            <p class="text-center">
                <a target="_blank"href="https://github.com/jrbrannen/Form-Handler-PHP.git">GitHub Repo Link</a>    <!--  GitHub Repo Link -->
            </p>
